...
def build_tree(data, current_depth=0, max_depth=1e10):
    ...
    for i in range(len(data[0])-1):
        for j in data:
            datas=divide_data(data,i,j[i])
            if best_gini>gini_impurity(datas[0],datas[1]):
                best_gini=gini_impurity(datas[0],datas[1])
                best_column=i
                best_values=j[i]
                best_split=(datas[0],datas[1])
    
    if abs(self_gini-best_gini)<1e-10:
        return DecisionNode(current_results=dict_of_values(data),is_leaf=True)
    else:
        return DecisionNode(true_branch=build_tree(best_split[0],current_depth+1,max_depth),
